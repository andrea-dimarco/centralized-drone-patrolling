...
typedef struct {
  ...
  modelica_integer _fifo_len;
  modelica_integer _msg_len;
...
static const MMC_DEFSTRUCTLIT(boxvar_lit_disconnect__from__db,2,0) {(void*) boxptr_disconnect__from__db,0}};
#define boxvar_disconnect__from__db MMC_REFSTRUCTLIT(boxvar_lit_disconnect__from__db)

/*
 * The function has annotation(Include=...>) or is builtin
 * the external function definition should be present
 * in one of these files and have this prototype:
 * extern double disconnect_from_db();
 */
 ...
 * the external function definition should be present
 * in one of these files and have this prototype:
 * extern int how_many_drones_in_area(int (*_area_id*), double (*_time_span*));
 */
