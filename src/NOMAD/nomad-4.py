...
def model(x):
	...
	ds[2] = (p_recharge -lb[2])/(ub[2]-lb[2])*100
	...
	if obj <= best:
		best = obj
		with open("best_result_nomad.txt", "w") as f:
...
# cc_choice, p_worst, p_recharge
x0 = [0.5, 0.05, 0.0015] # condizione iniziale
lb = [0.0, 0.01, 0.00] # lower-bound
ub = [1.0, 0.30, 0.01] # upper bound
